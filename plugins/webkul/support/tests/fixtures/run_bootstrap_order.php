...
//
// Usage: php run_bootstrap_order.php <json-encoded-array-of-plugin-names> [expected-database-name]
// Prints a single JSON object on success (exit 0): {"database": string,
// "tableCount": int, "fingerprint": string} — the fingerprint is
...
//
// The optional second argument is validated immediately after boot,
// before any plugin install runs (#138 PR4 review 4814881805): each test
// invocation now runs against its own ephemeral database, never the
// shared main test database, and this process must prove — not assume —
// that its actual DB connection matches exactly the database name the
// parent test asked for, rather than silently falling back to whatever
// DB_DATABASE the environment happened to already carry.
...
$expectedDatabase = $argv[2] ?? null;

try {
    $actualDatabase = DB::connection()->getDatabaseName();

    if ($expectedDatabase !== null && $actualDatabase !== $expectedDatabase) {
        throw new RuntimeException(
            "Expected to be connected to database \"{$expectedDatabase}\", but connected to \"{$actualDatabase}\" instead."
        );
    }
